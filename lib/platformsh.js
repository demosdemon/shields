...
const applyPlatformEnvironment = function(config) {
  ...
  if (
    ...
    config.relationships.rediscache &&
    config.relationships.rediscache.length
  ) {
    const rels = config.relationships.rediscache
    const hostname = rels.map(rel => `${rel.host}:${rel.port}`).join(',')
    const redisUrl = `redis://${hostname}`
